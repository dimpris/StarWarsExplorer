...
import { useGetFilmsDetailsQuery } from '../../services/redux/API.Service';
...
const FilmsDetails: FC = () => {
  ...
  const {data, isFetching, isError} = useGetFilmsDetailsQuery(itemId);
  
  let hasError = isError && data === undefined;
  let name = data && data.title ? data.title : '';

  const paramValues = Helpers.GetModelParamVaulesArray(data, ['episode_id']);
  
  return (
    <div className={ isFetching ? c_styles.Loading : c_styles.Loaded }>
      { hasError ? (<p className={c_styles.ErrorBox}>Error</p>) : '' }
      <h3>{name}</h3>
      <h4 style={{color: 'rgba(0, 0, 0, 0.6)'}}>(Films)</h4>
          ...
          <TableBody>
            {paramValues.map((p, i) => (
              <TableRow className={ i % 2 === 0 ? styles.RowEven : styles.RowOdd } key={'param'+i}>
                ...
                <TableCell style={{maxWidth: 600}}>{p.value}</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
