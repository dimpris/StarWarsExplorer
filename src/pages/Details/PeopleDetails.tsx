...
import { useGetPeopleDetailsQuery } from '../../services/redux/API.Service';
...
const PeopleDetails: FC = () => {
  ...
  const {data, isFetching, isError} = useGetPeopleDetailsQuery(itemId);
  ...
  return (
    <div className={ isFetching ? c_styles.Loading : c_styles.Loaded }>
      ...
      <h3>{name}</h3>
      <h4 style={{color: 'rgba(0, 0, 0, 0.6)'}}>(People)</h4>
